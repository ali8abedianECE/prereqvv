// src/features/scatter/ProfScatter.tsx
...
// Small row used in the live dropdown preview
...
export default function ProfScatter() {
    ...
    const [q, setQ] = React.useState<string>("");               // <-- always a string
    ...
    async function run(limit = 5000) {
        ...
        setErr(null);
        try {
            const res = await searchProfessors(q, limit);           // <- q is a string
            setData(res || []);
        } catch (e: any) {
    ...
    // initial full load
    React.useEffect(() => { run(5000); /* eslint-disable-next-line */ }, []);

    // build faculty/department lists
    ...
    // debounced server preview for the dropdown
    React.useEffect(() => {
        const t = setTimeout(async () => {
            const s = q.trim();
            if (!s) { setPreview([]); return; }
            try {
                const res = await searchProfessors(s, 12);
                setPreview(res || []);
            ...
            <Card>
                <H
                    right={
                        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", position: "relative" }}>
                                ...
                                {showPreview && preview.length > 0 && (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            borderRadius: 10,
                                            overflow: "hidden",
                                            boxShadow: "0 12px 24px rgba(0,0,0,.34)"
                                        }}
                                    >
                                        <div style={{
                                            display: "grid",
                            ...
                            {err && (
                                <span style={{ color: "#ff8a8a", whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden", maxWidth: 360 }}>
                  {err}
                </span>
                            )}
                        </div>
                ...
                <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 8, flexWrap: "wrap" }}>
                    ...
                    <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
                        <Button onClick={() => { setFaculty("All Faculties"); setDept("All Departments"); }}>
                            Reset Filters
                        </Button>
                    </div>
                </div>
